...
### 窗口生命周期要点
- `MainWindow`：
  - 首次显示与普通隐藏唤出使用 `WindowShowAnimation` 直接移动真实主窗口：从下方 18 DIP 开始，180ms 内缓出至上方 3 DIP，随后 80ms 平滑回落。保留窗口自身的背景、圆角和系统阴影，不创建预览窗口，也不改变透明度。动画期间暂时解除 Top 绑定，防止中间位置写入设置；完成或取消时恢复绑定并读取最新目标位置，兼容译文高度变化引起的位置调整。已有可见窗口再次唤起不重播，隐藏仍即时完成。
