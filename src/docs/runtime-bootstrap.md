...
### 窗口生命周期要点
- `MainWindow`：
  - 首次显示与普通隐藏唤出使用 `WindowShowAnimation` 的 260ms 淡入上浮：从下方 18 DIP 开始，180ms 内缓出至上方 3 DIP，随后 80ms 平滑回落；前 140ms 淡入。独立、无激活且不进 Alt+Tab 的 DWM 预览层承载整窗背景与内容，真实窗口仅临时 Cloak，位置绑定、布局和记忆坐标不参与动画。过渡层不设置 Owner，避免继承源窗口的遮蔽；动画期间非激活置顶，完成后关闭预览层并交还真实窗口。已有可见窗口再次唤起不重播，隐藏仍即时完成。
  - 首次显示在 `SourceInitialized` 提前遮蔽，`OnContentRendered()` 完成启动布局后播放；启动隐藏同样先遮蔽再隐藏，启动策略仅执行一次。输入、拖动、点击预览层、重复唤出、隐藏与退出均清理动画、释放缩略图并撤销 Cloak。普通唤出期间暂停贴顶吸附检测，贴顶展开沿用原有动画。遵循系统客户端区域动画开关，DWM 预览失败直接恢复正常显示，不新增配置项。
  - 预览层直接在源窗口所在屏幕创建，并设置 `WS_EX_NOACTIVATE`，避免跨屏/DPI 变化导致预览层激活；同时按源窗口尺寸和 DPI 更新圆角区域，避免动画中出现直角边缘。源窗口在动画期间忽略临时失焦，结束后恢复正常失焦隐藏。原生窗口定位会同步派发消息，可能重入隐藏/关闭流程；每次定位后检查动画是否已释放，取消后不再访问预览层，也不执行完成回调。
